...
static bool check_vector_faiss_build_threads(sys_var *self, THD *thd,
                                             set_var *var) {
  if (!check_vector_build_threads(self, thd, var))
    return false;
  return true;
...
static Sys_var_ulong Sys_vector_faiss_build_threads(
    "vector_faiss_build_threads",
    "Default OpenMP worker count for FAISS vector index build and rebuild. "
    "Use 0 for FAISS/OpenMP automatic sizing.",
    GLOBAL_VAR(opt_vector_faiss_build_threads), CMD_LINE(OPT_ARG),
    VALID_RANGE(0, vector_index::k_max_build_threads), DEFAULT(0),
    BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG,
